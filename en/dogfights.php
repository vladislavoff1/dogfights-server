...
function dogFight()
{
	...
	$enid = isset($_GET['enemy']) ? urldecode($_GET['enemy']): ''; 
...
//	print_r($fightResult); echo "<br>";
	//echo $fightResult.'<br>';
	preg_match('/<win>([0-9]{1,20})<\/win>/is', $fightResult, $win);
...
//print_r($enemy); echo "<br>";
	...
	if ($uid == $win[1]) {
		...
		$result = '1';
		$resmsg = TXT_DOG_WIN_FIGHT;
	} else {
	//	echo "LOSE"; echo "<br>";
		$result = '0';
		$resmsg = TXT_DOG_LOSE_FIGHT;
	}
...
function getInfo($userid)
{
	Global $conn;
/*	
	$query = "SELECT `users_dogs`.`user_id` as id, `users_dogs`.`DogHealth`, `users_dogs`.`strength`, `users_dogs`.`dexterity`, `users_dogs`.`endurance`, `users_dogs`.`DogLevel`, `users_dogs`.`DogName`
                FROM `users_dogs` 
	...
	$query2 = "SELECT IF(`items`.`type` = '4',SUM(`items`.`rate`),0) as `coef`
                FROM `used_items` 
                INNER JOIN `items` ON `items`.`id` = `used_items`.`item_id`
                WHERE `used_items`.`user_id` = '{$data['id']}'
	...
	if(mysql_error($conn)) trigger_error(MSG_PROBLEM_WITH_SQL);
	$data2 = mysql_fetch_array($rez);
//print_r($data2); echo "<br>";

	$data['coef'] = $data2['coef'];
	$data['VKuser'] = $userid;
*/
	$query = "SELECT `users_dogs`.`user_id` as id, `users_dogs`.`DogHealth`, `users_dogs`.`strength`, `users_dogs`.`dexterity`, `users_dogs`.`endurance`, IF(`items`.`type` = '4',SUM(`items`.`rate`),0) as `coef`, `users_dogs`.`DogLevel`, `users_dogs`.`DogName`
                FROM `users_dogs` 
...
//print_r($query);
	...
	if ($data != '') return $data;
	else trigger_error(MSG_NO_USER_ID);
}
...
/**
 * UPDATE expire time for doping items
 *
 * @param int $uid
 * @return unknown
 */
...
//	$damage = ($dog1['health']+$dog2['health'])/100*$power*$cf*$cv; //3 - ������ ����������
	...
	return array(
		...
		'pDamage' => 100*$damage/($oldHealth+1). '%',
	);
}
/**
...
foreach ($aBattle as $key => $stroke) {
	...
	$responce .= '<stroke>'.$aBlow[$stroke[1]['stroke']]['id'].'</stroke>';
...
//	$responce .= '<oldhealth>'.$stroke[1]['oldHealth'].'</oldhealth>';
	...
	if (isset($stroke['contr2'])) {
		...
		$responce .= '<stroke contr="1">'.$aBlow[$stroke['contr2']['stroke']]['id'].'</stroke>';
...
//		$responce .= '<oldhealth>'.$stroke['contr2']['oldHealth'].'</oldhealth>';
	...
	if (isset($stroke[2])) {
		$responce .= "<dog id='{$enemy['VKuser']}'>";
		$responce .= '<stroke>'.$aBlow[$stroke[2]['stroke']]['id'].'</stroke>';
...
//		$responce .= '<oldhealth>'.$stroke[2]['oldHealth'].'</oldhealth>';
	...
	if (isset($stroke['contr1'])) {
		$responce .= "<dog id='{$user['VKuser']}'>";
		$responce .= '<stroke contr="1">'.$aBlow[$stroke['contr1']['stroke']]['id'].'</stroke>';
